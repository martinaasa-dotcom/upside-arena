...
/*
  ...
  Every lobe in this app is a low alpha wash on a true black page, so the
  whole ramp lives in the bottom of the range: about thirty luminance
  levels of 255 across most of a screen. Thirty levels over eight hundred
  ...
  the whole product is built on, too large a one darkens the lobes.
  ...
  zero, only the positive half survives, and every unlit pixel ends up half
  a level above black. Measured with the clip in place, a true black patch
  on this page reads 0.00 with a spread of 0.00 after the dither, and a lit
  patch went from a spread of 1.40 to 2.43 with its mean moving 0.08.
  ...
  quietly renders undithered.

  The same file, the same numbers and the same guard live in Upside Lab.
  The two apps are one design, so fix both or neither.
*/
...
describe("the ambient dither", () => {
  ...
  it("keeps the amplitude at two levels of 255", () => {
    ...
    expect(Math.round(alphaRowOf(matrices[1]) * 255)).toBe(2);
  });
  ...
  it("is one octave of grain, not a cloud", () => {
    ...
    expect(Number(/baseFrequency="([\d.]+)"/.exec(FILTER)?.[1])).toBeGreaterThanOrEqual(0.5);
  });
...
describe("surfaces that ramp through near-black", () => {
  for (const selector of DITHERED) {
    it(`${selector} carries the dither`, () => {
      expect(ruleFor(selector)).toContain("url(#ambient-dither)");
    });
  }

  it("gives .ambient-glow a falloff curve rather than two stops", () => {
    const stops = [...ruleFor(".ambient-glow").matchAll(/oklch\(from var\(--primary\)/g)];
    expect(stops.length, "a colour-to-transparent ramp has an edge you can see")
  ...
  it("every profile theme inherits the dither from the base rule", () => {
    // The themes override the gradients only. If one ever grows its own
    // `filter` it silently drops the dither for everybody using it.
    for (const [, body] of CSS.matchAll(
      /\[data-arena-theme="[^"]+"\]\s*\.page-frame::before\s*\{([^}]*)\}/g
    )) {
