/*
  ...
  Upside Lab has the same component and the same test. Fix both or neither.
*/
...
describe("the switch that made this necessary is still thrown", () => {
  ...
  it("is mounted outside the element it moves", () => {
    /* The notes in that file mention `<main>`; the element is what counts. */
    const before = LAYOUT.indexOf("<PullToRefresh />");
    const main = LAYOUT.indexOf('<main id="main"');
    expect(before).toBeGreaterThan(-1);
    expect(before, "a fixed ring inside a transformed main is re-parented")
      .toBeLessThan(main);
  });
});
