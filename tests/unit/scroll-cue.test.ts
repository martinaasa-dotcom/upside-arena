/*
  ...
  136px off the bottom of the screen, which is the loudest continuation cue
  ...
  scroll. Measured at 412x915 with the CPU throttled ten times, one pass
  down this page presented 42 frames the compositor had to repaint, the
  ...
  screen sizes where it matters. Upside Lab's copy is the same component
  under the same rules, in `src/lib/scroll-cue.test.ts`. Fix both or neither.
*/
...
describe("the cue stands down when the page is already saying it continues", () => {
  ...
  it("re-measures rather than guessing from a breakpoint", () => {
    /*
      ...
      and a font swap moves more than that, measured on Upside Lab with the
      card sitting 11px clear of the band.
    */
  ...
  it("stands down while the measurement question holds the same line", () => {
    // Below `sm` that question is a full-width strip across this exact line.
