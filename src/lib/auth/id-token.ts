/*
  ...
  is not something a browser handed us: Arena's server fetched it from
  Google's own token endpoint over TLS, in exchange for a single use code
  bound to the state cookie this server set, using a client secret that never
  leaves the server. OpenID Connect says in as many words that a token
  received directly from the token endpoint over a protected channel does not
  need its signature validated, and this is that case exactly.

  What is checked is everything that says the token is about this app and is
  still good: who issued it, who it was issued for, whether it has expired, and
  whether Google considers the address itself confirmed. An unconfirmed address
  is refused outright, because the whole point of what happens next is that
  holding this address proves something.
...
export function googleEmailFromIdToken(
  ...
  // Google sends this as a boolean in the token and as a string in some of its
  // other responses. Both mean the same thing and anything else is a no.
