/*
  Whether an address can plausibly receive a sign-in link.

  Arena's only email is a magic link, so a mistyped address is not a small
  problem. Nothing arrives, the person assumes the app is broken, and the
  message hard-bounces at the far end. Enough of those and the sending
  provider throttles or suspends the project, which takes sign-in down for
  everybody, including the people who typed their address correctly.
...
/*
  ...
  guaranteed bounce, and they turn up in real sign-in fields constantly,
...
/*
  Mailboxes that exist to send and never to receive. A link posted to one of
  these is either rejected outright or read by nobody, and in both cases the
  person waiting for it is waiting for nothing.
*/
...
/*
  ...
  likely to be one edit away from. The Estonian ones are here because Arena's
  first players are, and a suggestion list that only knows Gmail is no help to
  somebody on hot.ee.
*/
...
/*
  ...
  `"a b"@[192.0.2.1]` into a sign-in box, and every address that looks like it
  in practice is a paste that went wrong.
*/
 ...
 * things somebody meant to leave out, so removing them is not correcting them.
 * Case is not information in a domain, and no mail system anybody uses treats
 * it as information in a local part either.
 */
...
function stem(domain: string): string {
  ...
  return labels[Math.max(0, labels.length - 2)];
}
...
/**
 * Reads a typed address and says whether a sign-in link should go to it.
 *
 ...
 * unusual is allowed through, because a rule that keeps out one real person to
 * stop one bounce is a bad trade for a product nobody has signed up to yet.
 */
export function readEmail(raw: string): EmailVerdict {
  ...
  if (UNATTENDED_LOCAL_PARTS.has(local)) {
    return {
      kind: "unreachable",
      email,
      message: "That mailbox only sends, so a sign-in link would never reach anybody.",
    };
  }
...
/** Whether mail may be sent to an address held on file, with no chance to ask about it. */
export function isSendable(email: string | null | undefined): boolean {
  if (!email) return false;
  return readEmail(email).kind !== "unreachable";
}
