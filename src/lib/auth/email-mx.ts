...
/*
  ...
  listening, and a link sent there bounces without exception. One lookup before
  the send is what turns a bounce into a sentence on the sign-in form.
  ...
  because refusing to sign somebody in over a slow DNS server would be a far
  worse fault than the bounce this exists to prevent.
*/

const LOOKUP_TIMEOUT_MS = 2_500;

/** Answers are held so a busy sign-in page does not re-ask about gmail.com all day. */
const YES_TTL_MS = 6 * 60 * 60 * 1000;

/*
  A no is held for minutes rather than hours. A domain that was just registered,
  or one whose records were being edited while somebody tried to sign in,
  should not be turned away for the rest of the day over it.
*/
...
export async function domainAcceptsMail(domain: string): Promise<boolean> {
  ...
  try {
    ...
    /*
      An empty answer, and a single exchanger pointed at the root, are both the
      published way of saying "this domain receives no mail". Rare, and a
