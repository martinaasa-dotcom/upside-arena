...
export function emailHtml(message: Message, origin: string, unsubscribeUrl: string) {
  ...
  /*
    Inline styles and the sRGB conversions of the tokens, because a mail
    client will not load a stylesheet or understand oklch. The values come
    from one place so that moving the accent moves it here too, which is
    exactly what a hardcoded copy failed to do once already.
  */
  return `<!doctype html>
<html><body style="margin:0;padding:24px;background:#000000;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <table role="presentation" width="100%" style="max-width:480px;margin:0 auto;background:#171717;border-radius:10px;padding:24px;">
    <tr><td>
      ...
      <a href="${link}" style="display:inline-block;background:${HEX.primary};color:${HEX.primaryForeground};text-decoration:none;padding:10px 18px;border-radius:999px;font-size:15px;font-weight:600;">Open Arena</a>
