...
/*
  ...
  AND WHY THE GESTURE IS WORTH HAVING AT ALL HERE. Every room is a server
  component, so nothing on a screen that is already open ever refetches: the
  figures are as old as the last navigation, `PriceAge` says so honestly in
  the corner, and the app runs standalone, which means there is no address
  bar and therefore no reload button anywhere in it. A player watching their
  week on a Friday afternoon had no way at all to ask for the current number.
  Now they have the one every phone has taught them.
  ...
  Upside Lab has the same component, the same curve and the same ring. The
    ...
    */
    ...
    const onStart = (e: TouchEvent) => {
      ...
      /*
        ...
        from the route. Upside Lab keeps every room it has visited mounted and
        hidden, so there is more than one `<main>` in that document and only
        the one the finger is actually inside is the right one to move. Here
        there is only ever one, and asking the same question costs the same.

        It also settles what happens on the header: a pull that starts on the
        chrome is not a pull, which is the same answer a native list gives.
      */
...
/*
  What a pull actually asks for here: the room again, from the server.

  Every room is a server component, so this is the whole refresh. `isPending`
  is what tells the ring when to stop, because it is the only thing that knows
  the new payload has arrived and been drawn. The ring's own ceiling covers
