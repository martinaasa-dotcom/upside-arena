...
/*
  ...
  and it stays exactly where it is while everything it is pointing at moves.

  AND IT WAS THE REASON THE PAGE FILLED IN AS YOU SCROLLED IT. The pill was
  `glass-notice`, which is a `backdrop-filter`, and a backdrop-filter fixed
  over content that is moving has to re-filter what is behind it every
  single frame. Measured on a 412x915 screen at ten times CPU throttling,
  scrolling the signed-out page presented 42 frames that the compositor had
  to repaint, and in the worst of them 38% of the bottom eighth of the
  screen had not caught up with where the page actually was. Hiding this one
  element and changing nothing else took that to 9 frames, every one of them
  pixel-identical to the settled page. The bottom band of the window is
  exactly where new content arrives while you scroll, which is why what a
  reader saw was the lower half of the sample card appearing late.
  ...
  the fault above. `tests/unit/scroll-cue.test.ts` holds it to that.
  ...
  Same component, same words and same rules as Upside Lab's. The two apps
...
/*
  ...
  1. THE SAMPLE CARD IS CUT BY THE FOLD. Content visibly severed by the
     ...
     every phone and most laptops that is what happens: measured at 412x915
     the card runs 136px past the bottom edge, at 390x844 it runs 237px
     past. Nothing is drawn. This is the case the old pill got wrong on
     every phone anybody owns, where it sat over a card that was already
     shouting.

  2. THE NEXT SECTION HAS REACHED THE SCREEN. On a tall display the hero's
     own height floor leaves 96px of the following section's heading showing,
     so the reader is already looking at the page continuing. Nothing is
     drawn, and any amount of it counts: a heading the fold cuts says the
     same thing the sample card does, and a line of type laid over a heading
     arriving at the bottom of the screen would be worse than no line at all.
  ...
  3. THE FIRST SCREEN ENDS CLEAN. The card clears the fold whole and nothing
     ...
     measured against this page it is windows roughly 960 to 1016 CSS pixels
     tall, which is about what a maximised browser gets on a 1080p display.
 ...
 * page does not keep. It also keeps the measurement question's own reserved
 * space from being mistaken for content: an unanswered one pads the bottom
 * of the frame by several rem, which is real scroll height with nothing in
 * it.
 */
const RUNWAY = 240;
...
/*
  ...
  THIS IS WHY THE CUE WAS MISSING ON A RELOAD AND ARRIVED ON A FLICK. All of
  this used to be read off `getBoundingClientRect`, which reports where a
  thing is being *drawn*, and the first thing everything on this page does is
  arrive: `.rise` holds the sample card 8px below where it lands, with `both`
  fill, from before the first frame until 0.76s in, and an `Arrive` section
  sits 12px low for as long as it is out. The decision is made once, at
  hydration, well inside that window, so the card was measured lower than it
  really is and the cue stood down as though the fold were cutting it.
  Nothing then re-measured for the rest of the animation, so the page sat
  there saying nothing, and the first thing to run the measurement again was
  the reader scrolling, which is the one moment the answer no longer matters.
  Measured on Upside Lab's copy of this page, which is the same component
  with a 12px rise: no cue at all on any window between 950px and 961px tall,
  and one wheel notch brought it up.
  ...
  first frame is the answer at the last one, and it is right before the page
  has finished arriving rather than after. Walking `offsetParent` up to the
  ...
  useEffect(() => {
    ...
    const read = () => {
      ...
      /*
        ...
        ends up somewhere other than the bottom of the first screen, nothing
        is drawn rather than something being drawn in the wrong place.
      */
    ...
    /*
      ...
      The first read is the one that counts, and measuring the layout rather
      than the paint is what makes it right: the answer at hydration is the
      answer the settled page has, so the cue is there on a reload rather
      than waiting for something to happen. See `pageTop` for what reading
      it the other way cost.

      The rest are for the page changing shape under it, which it still
      does. This decides on a few pixels of clearance, and a font swap moves
      more than that, as does the sign-in button filling in where a 44px
      space was. So: the resize the reader causes, the scroll that answers
      the question, the body growing for any reason at all, the fonts
      arriving, and load. Reading is a handful of offsets and no writes, so
      re-running it costs nothing worth counting.
    */
    read();
    ...
    >
      <button
        ...
        className={cn(
          ...
          /*
            Below `sm` the measurement question is a full-width strip across
            the bottom of the window, on top of this line. It is the louder
            of the two and it is asking something, so this stands down until
            it has been answered. From `sm` up that question is a card in
            the right-hand corner and this is centred, so they never meet.
          */
